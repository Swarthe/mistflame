...
- **The generated HTML is sent but deliberately not stored.** Nothing needs it: our own messages display as plain text (see "HTML email rendering"), and a reply always parents an inbound email because the `+ Reply` button only appears on those, so the HTML a reply nests is always the contact's and never ours. Keeping it out of the row also keeps the post-send body rewrite small; if that `UPDATE` fails the row stays marked sent (the claim wrote `sent_at` before the send, see "Recipient delivery"), just without the quote appended to its stored body.
...
- `src/components/`: `ContactSidebar`, `ContactForm`, `EmailCard`, `NewEmailCard`, `EmailFrame`, `SendModal`, `ForwardModal`, `RecipientField`, `TagChip`, `ReplyPreview`, `AttachmentChip`, `SnippetText`, plus `styles.ts` with the shared Tailwind class strings.
