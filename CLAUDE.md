...
- `src/components/`: `ContactSidebar`, `ContactForm`, `EmailCard`, `NewEmailCard`, `BodyEditor`, `EmailFrame`, `SendModal`, `ForwardModal`, `RecipientField`, `TagChip`, `ReplyPreview`, `AttachmentChip`, `SnippetText`, plus `styles.ts` with the shared Tailwind class strings. `BodyEditor` is the markdown body editor shared by `NewEmailCard` and `EmailCard`'s edit mode: the formatting toolbar, Write/Preview tabs, keyboard shortcuts and Enter-key list continuation.
